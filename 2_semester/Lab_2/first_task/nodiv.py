...
if(scheme == "L"):# Lux scheme
    ...
    for k in range(K):
        for n in range(1, N):
            z = u[k, n]
            for s in range(7):
                z = z - (z - u[k, n] + 0.5 * tau * z * (u[k, n + 1] - u[k, n - 1] - tau * z * (u[k, n + 1] - 2 * u[k, n] + u[k, n - 1]) / h) / h) / \
                (1 + 0.5 * tau * (u[k, n + 1] - u[k, n - 1]) / h - tau**2 * z * (u[k, n + 1] - 2 * u[k, n] + u[k, n - 1]) / h**2)
            u[k + 1, n] = z
    ...
    for k in range(K):
        for n in range(0, N -1):
            u[k + 1, n] = u[k, n] * (1 - tau * (u[k, n] - u[k, n - 1]) / h)
